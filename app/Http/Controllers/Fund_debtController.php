...
class Fund_debtController extends Controller
{
  public function __construct()
  {
    ...
    /* $u = department::create([
        ...
        return redirect()->route('Fund_debt.index')->with('message','item has been added successfully');
     }
    ...
    {
        return view('Fund_debt.edit',compact('service'));
    }
    ...
    public function update(ServiceRequest $request,Service $service)
   {
    //  $affectedRows = de::where('id',$de->id)->update(['name' => $de->name,'email' => $de->email,'role' => $de->role ]);
     ...
     {
        $service->delete();
        return redirect()->route('Fund_debt.index')->with('message','item has been deleted successfully');
     }
}
